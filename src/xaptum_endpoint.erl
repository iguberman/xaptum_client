...
-callback auth(Host :: list(), Port :: int(), Creds :: term(), CallbackData :: any())->
  {Identity :: binary() , Cert :: binary(), Key :: binary()}.
-callback on_receive(Msg :: binary(), Parent :: pid(), CallbackData :: any()) -> {ok, CallbackData :: any()}.
-callback receive_loop(TlsSocket :: tuple(), Parent :: pid(), CallbackData :: any()) -> ok.
-callback on_send(Msg :: binary(), Dest :: any(), Parent :: pid(), CallbackData :: any()) ->
  {Msg :: binary(), CallbackData :: any()}.
-callback on_send(Msg :: binary(), Parent :: pid(), CallbackData :: any()) ->
  {Msg :: binary(), CallbackData :: any()}.
-callback on_disconnect(CallbackData :: any()) -> ok.
-callback on_connect(Parent :: pid(),  CallbackData :: any()) -> {ok, CallbackData :: any()}.
-callback on_connect_retry(Parent :: pid(), CallbackData :: any()) -> {ok, Callbackdata :: any()}.
...
set_data(EndpointPid, NewData)->
  gen_server:call(EndpointPid, {set_data, NewData}).
...
init([CallbackModule, CallbackData, Creds, XaptumHost, XaptumPort]) ->
...
handle_call({set_data, NewData}, _From, State)->
  {reply, ok, State#state{callback_data = NewData}}.
...
terminate(_Reason, #state{tls_socket = undefined} = State) ->
  ok;
terminate(_Reason, #state{tls_socket = TlsSocket, callback_module = CallbackModule, callback_data = CallbackData}) ->
  erltls:close(TlsSocket),
  CallbackModule:on_disconnect(CallbackData),
  ok.
