...
if [[ -f $PK_BIN && -f $SK_BIN ]]; then
   echo "File $PK_BIN and $SK_BIN exist"
    ...
    ecdaa_member_request_join $NONCE $PK_BIN $SK_BIN
fi
...
## GENERATE CREDENTIAL
ecdaa_issuer_respond_to_join_request $PK_BIN $ISK_BIN $CRED_BIN $CRED_SIGN_BIN $NONCE
...
## SIGN AND VERIFY
ecdaa_member_sign $SK_BIN $CRED_BIN $SIG_BIN $MSG_BIN $BASENAME_BIN
